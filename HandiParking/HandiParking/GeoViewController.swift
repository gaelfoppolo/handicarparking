...
class GeoViewController: UIViewController, CLLocationManagerDelegate, GMSMapViewDelegate, UIActionSheetDelegate {
    ...
    */
    ...
    /**
    Appelée juste avant que l'instance soit désalloué de la mémoire. Ainsi on supprime l'observateur avant de désallouer l'instance et l'application ne crash pas en désallouant map
    */
    deinit {
    ...
    */
    ...
    func makeStringSearchresultsInRadius() -> NSString {
        var string = "\(parkingSpaces.count) place"
        if parkingSpaces.count > 1 {
            string += "s"
        }
        string += " dans un rayon de "
        if radius.value > 500 {
            string += "\(radius.value/1000) km"
        } else {
            string += "\(radius.value) m"
        }
        return string
    }
    ...
    func createMarkersAndBoundsToDisplay() {
        ...
        var bounds = GMSCoordinateBounds(coordinate: sourceOfSearch(), coordinate: sourceOfSearch())
            ...
            UIView.animateWithDuration(2.5, animations: {
                self.resultsInRadius.alpha = 0.75
            })
